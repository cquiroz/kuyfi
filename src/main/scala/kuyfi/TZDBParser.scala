...
object TZDBCodeGenerator {
  ...
  object TreeGenerator {
    ...
    implicit val zoneListInstance: TreeGenerator[List[Zone]] =
      instance(z => {
        val objectBlock: List[Tree] = if (z.nonEmpty) {
              List(LAZYVAL("allZones", "List[String]") := LIST(z.map(_.toTree)))
            } else {
              Nil
            }
  ...
  def exportAll(rows: List[Row]): Tree = {
    BLOCK (
      OBJECTDEF("tzdb") := rows.flatMap(_.select[Zone]).toTree
