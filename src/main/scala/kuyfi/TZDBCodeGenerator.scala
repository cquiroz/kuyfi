  ...
  def exportTzdb(tzdbPackage: String, importsPackage: String, rows: List[Row])
    ...
    BLOCK (
      List(
        ...
        OBJECTDEF("tzdb") := BLOCK(IMPORT("tpe._") :: List(OBJECTDEF("rules") := BLOCK(standard.map(_.toTree).toList), fixed.toTree, standard.toTree, rows.flatMap(_.select[Link]).toTree)))
