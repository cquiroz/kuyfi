...
object TZDBCodeGenerator {
  ...
  val zoneOffsetSym = getModule("ZoneOffset")
  val zoneRulesSym = getModule("ZoneRules")
  ...
  def exportAll(zonePackage: String, rows: List[Row]): Tree = {
    BLOCK (
      List(
        IMPORT(s"$zonePackage._"),
        IMPORT(s"$zonePackage.zone._"),
