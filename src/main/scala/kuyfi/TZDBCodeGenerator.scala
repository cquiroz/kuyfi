  ...
  object PureTreeGenerator {
    ...
    implicit val stdListInstance: TreeGenerator[List[(Zone, StandardRulesParams)]] =
      TreeGenerator.instance { l =>
        LAZYVAL("stdZones", TYPE_MAP(StringClass, TYPE_REF("ZR"))) := MAKE_MAP(l.map {
          case (z, _) => TUPLE(List(LIT(z.name), REF(z.scalaSafeName)): _*)
        })
      }

    implicit val fixedListInstance: TreeGenerator[List[(Zone, FixedZoneRulesParams)]] =
      TreeGenerator.instance(l =>
        LAZYVAL("fixedZones", TYPE_MAP(StringClass, TYPE_REF("ZF"))) := MAKE_MAP(l.map {
          case (z, _) => TUPLE(List(LIT(z.name), REF("rules." + z.scalaSafeName)): _*)
        })
      )
