...
object TZDBCodeGenerator {
  ...
  object PureTreeGenerator {
    ...
    implicit val version: TreeGenerator[TzdbVersion] =
      TreeGenerator.instance(v =>
        LAZYVAL("version", TYPE_REF("String")) := LIT(v.ver)
      )
  }
  ...
  def exportTzdb(version: TzdbVersion, tzdbPackage: String, importsPackage: String, rows: List[Row], zoneFilter: String => Boolean)(implicit genVersion: TreeGenerator[TzdbVersion], genRuleMap: TreeGenerator[(Zone, StandardRulesParams)], genFixedList: TreeGenerator[List[(Zone, FixedZoneRulesParams)]], genStdList: TreeGenerator[List[(Zone, StandardRulesParams)]], genLinks: TreeGenerator[List[Link]]): Tree = {
    ...
    val totalFilter: String => Boolean = n =>
      linkNames.contains(n) || zoneFilter(n)

    // Fixed zone rules
    val fixed: List[(Zone, FixedZoneRulesParams)] = rules.filter(x => totalFilter(x._1.name)).collect {
      case (z, r: FixedZoneRulesParams) => (z, r)
    }.toList
    // Standard zone rules
    val standard: List[(Zone, StandardRulesParams)] = rules.filter(x => totalFilter(x._1.name)).collect {
    ...
    BLOCK (
      List(
        ...
        OBJECTDEF("tzdb") := BLOCK((version.toTree :: groupedRules) ::: List(fixed.toTree, standard.toTree, links.toTree)))
    ) inPackage tzdbPackage withComment autoGeneratedCommend
  }

  def exportAll(dir: java.io.File, to: java.io.File, packageName: String, importsPackage: String, zoneFilter: String => Boolean)
    (implicit genVersion: TreeGenerator[TzdbVersion], genRuleMap: TreeGenerator[(Zone, StandardRulesParams)], genFixedList: TreeGenerator[List[(Zone, FixedZoneRulesParams)]], genStdList: TreeGenerator[List[(Zone, StandardRulesParams)]], genLinks: TreeGenerator[List[Link]]): IO[better.files.File] = {
