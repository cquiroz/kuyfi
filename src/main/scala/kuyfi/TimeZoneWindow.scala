...
object TimeZoneWindow {
  ...
  final case class RulesTimeZoneWindow private (standardOffset: GmtOffset, windowEnd: LocalDateTime, timeDefinition: TimeDefinition, fixedSavingAmountSeconds: Option[Int], windowRules: WindowRules) extends TimeZoneWindow {
    ...
    override def tidy(windowStartYear: Int): TimeZoneWindow = {
      val newWindowRules = if (windowEnd == LocalDateTime.MAX) {
        ...
        val lastRulesPlus1 = windowRules.lastRuleList.map(_.copy(to = GivenYear(maxLastRuleStartYear + 1)))
        val sr = RulesTimeZoneWindow.addRules(windowRules, lastRules).copy(lastRuleList = lastRulesPlus1,  maxLastRuleStartYear = Year.MAX_VALUE)
        ...
        RulesTimeZoneWindow.addRules(windowRules, lastRules).copy(lastRuleList = Nil, maxLastRuleStartYear = Year.MAX_VALUE)
      }
      val newFixedAmountSecs = if (newWindowRules.ruleList.nonEmpty || fixedSavingAmountSeconds.isDefined) {
        fixedSavingAmountSeconds
      } else {
        None
      }
      RulesTimeZoneWindow(standardOffset, windowEnd, timeDefinition, newFixedAmountSecs, newWindowRules.sorted)
    }
  }
  ...
  object RulesTimeZoneWindow {
    ...
    private def addRules(originalRules: WindowRules, rules: List[Rule]): WindowRules = rules.foldLeft(originalRules) { (acc, rule) =>
    ...
    def apply(standardOffset: GmtOffset, windowEnd: LocalDateTime, timeDefinition: TimeDefinition, fixedOffsetSec: Option[Int], rules: List[Rule]): RulesTimeZoneWindow =
      RulesTimeZoneWindow(standardOffset, windowEnd, timeDefinition, fixedOffsetSec, addRules(rules))
  }
