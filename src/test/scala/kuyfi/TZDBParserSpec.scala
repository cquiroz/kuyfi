...
class TZDBParserSpec extends FlatSpec with Matchers {
  ...
  "TZDBParser" should
    ...
    it should "parse a whole dir" in {
      ...
      //rows.flatMap(_.select[Zone]).filter(_.transitions.exists(_.rules.id.startsWith("-"))).map(u => (u.name, u.transitions.filter(_.rules.id.startsWith("-")).map(_.rules.id))).sortBy(_._1).foreach(println)
      rows.flatMap(_.select[Zone]).map(u => (u.name, u.transitions.map(_.ruleId).distinct)).sortBy(_._1).foreach(println)
      println("----------")
